...
const char *set_flags(const char *format, Flags *flags) {
  ...
  case '+':
    ...
    break;
  case ' ': {
    if (*(format - 1) == '%' && (s21_is_digit(*(format + 1)) ||
                                 *(format + 1) == '-' || *(format + 1) == '+'))
      flags->space = 1;
  } break;
  case '0': {
...
char *spec_decimal(long long int var_len, Flags flags, char *str) {
  ...
  if (var_len == 0)
    size = 1;
  if ((s21_size_t)flags.width > size)
    size = flags.width;
  if ((s21_size_t)flags.precision > size)
  ...
  if (decimal_str != s21_NULL)
    free(decimal_str);
  decimal_str = s21_NULL;
  *str = '\0';
  return str;
}
...
int s21_itoa(Flags flags, long long int var_len, s21_size_t size,
  ...
  if ((flags.plus == 1) && (var_len >= 0) &&
      (((flags.precision < flags.width) && ((s21_size_t)i < size)) ||
       ((s21_size_t)i == size)))
    buf[i++] = '+';
  if ((var_len < 0) && ((s21_size_t)i == size))
    buf[i++] = '-';
...
char *spec_float(double var_len, Flags flags, char *str) {
  ...
  if (integ == 0)
    size_i = 1;
  if ((s21_size_t)flags.precision > size_fr)
    size_fr = flags.precision;

  s21_size_t size = size_fr + size_i;
  size++;
  if ((var_len < 0) || (flags.space) || (flags.plus))
    size++;
  if ((flags.precision == 0) && (flags.is_precision))
    size--;
